...
instance_refresh {
  strategy = "Rolling"
  preferences {
    #instance_warmup = 300 # Default behaviour is to use the Auto Scaling Group's health check grace period.alarm_specification {
      min_healthy_percentage = 50
    }
    triggers = [ /*"launch_template",*/ "desired_capacity" ] # You can add any argument from here, if those has changes, ASG Instance Refresh will trigger
  }
tag {
    key = "Name"
    value = "frontend-asg"
