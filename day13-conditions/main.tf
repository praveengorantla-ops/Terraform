...
resource "aws_s3_bucket" "dev" {
    bucket = "statefile-configuress"


}

#after run this will get error like the variable 'aws_region' must be one of the following regions: us-west-2, | eu-west-1, so it will allow any one region defined above in condition block

# example-4 with numeric condition in this condition if ec2 instance = t2.micro only instance will create (count = var.instance_type == "t2.micro" ? 1 : 0) but i am passing t2.nano so ec2 will not create 
#variables.tf
# variable "ami" {
#     type           = string
